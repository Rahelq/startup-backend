...
exports.create = async (req, res) => {
  try {
    const body = req.body || {};
    const host_id = body.host_id || req.user.user_id;
    const participant_id = body.participant_id;
    if (!participant_id) return res.status(400).json({ message: "participant_id required" });

    // simple access: host must be the authenticated user
    if (host_id !== req.user.user_id)
      return res.status(403).json({ message: "You must be the host to create a session as host" });
    ...
    // Emit real-time video session created event
    realtimeEmitter.emitVideoSessionStarted(
      session.id,
...
exports.reschedule = async (req, res) => {
  try {
    ...
    // Emit real-time reschedule event
    realtimeEmitter.emitVideoSessionRescheduled(id, updated.scheduled_at, [
      s.host_id,
      s.participant_id,
    ]);
...
exports.cancel = async (req, res) => {
  try {
    ...
    // Emit real-time video session ended event
    realtimeEmitter.emitVideoSessionEnded(id, [s.host_id, s.participant_id], 0);
...
exports.join = async (req, res) => {
  try {
    ...
    // Emit real-time participant joined event
    const otherUserId = s.host_id === req.user.user_id ? s.participant_id : s.host_id;
    realtimeEmitter.emitVideoParticipantJoined(id, req.user.user_id, [otherUserId]);
...
exports.zoomWebhook = async (req, res) => {
  try {
    ...
    const updated = await videoSessionService.handleZoomWebhookEvent(eventName, req.body);
